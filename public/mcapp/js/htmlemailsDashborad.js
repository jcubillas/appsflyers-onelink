...
function buildDashboard(emails, from, page) {
  ...
  if (emails !== undefined) {
 /*   emails.sort((a, b) => ((new Date(a.Modified) < new Date(b.Modified)) ? 1 : ((new Date(b.Modified) < new Date(a.Modified)) ? -1 : 0)));
    if (from == "init" || page == 1) {
      emails = emails.splice(0, 15);
  ...
  for (let index = 0; index < emails.length; index++) {
    ...
    document.getElementById(`link${index}`).addEventListener("click", function (e) {
      ...
      var links = getLinks(rawHTML);
       ...
       console.log(replaceLinks(rawHTML,arrObj));
    });
  }
}
